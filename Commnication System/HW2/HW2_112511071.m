...
%% ---------- 1️. Sampling ----------
n = 1:Ns:length(m);    % sampling period is Ns
m_s = m(n);
...
m_min = min(m_s);
m_max = max(m_s);
delta = (m_max-m_min)/(L-1); %quantization interval
            %devide interval [m_min, m_max] into L-1 part

q_index = round((m_s-m_min)/delta);  %quantization index, each value 0~L-1
               %try the function "round" 
a = m_min+delta*q_index;  %quantization value
...
%% ---------- 5️. Matched Filter ----------
h = fliplr(pulse);
y = conv(r_pam,h);  %pass the signal r_pam through filter h
                    %try function "conv"
y = y / sum(pulse.^2);
t_y = (0:length(y)-1)/fd;
...
for k = 1:length(a)
    a_hat(k) = round((sample_points(k)-m_min)/delta)*delta+m_min;
...
for k = 1:length(a_hat)
    m_rec_sinc = m_rec_sinc + a_hat(k) * sinc((t - t_sample(k)) * fs);
end
